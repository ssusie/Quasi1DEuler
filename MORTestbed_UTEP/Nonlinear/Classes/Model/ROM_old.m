...
%         end %Done
        ...
        function  [] = NewtonRaphson(obj)
            ...
            w_guess = obj.phi'* obj.sv(:,itnump1); %initial guess for generalized coordinates for Newton iteration
...
%                 H3=hessian(@obj.constr3,w_guess);
        ...
        function [eflax, deflax]=eulerflux_a(obj, w)
                ...
                U=reshape(obj.phi*w,3,obj.prob.nVol); 
                rho(1)=U(1,1); u(1)=U(2,1); P(1)=U(3,1);               
                e(1)= P(1)/(obj.prob.gamma-1)+rho(1)*u(1)^2/2;                
                c(1) = sqrt(obj.prob.gamma*P(1)/rho(1)); %Speed of sound at boundary a                
                flax_a(1,1) = rho(1)*u(1); flax_a(2,1)=rho(1)*u(1)*u(1)+P(1); 
                flax_a(3,1) = (e(1)+P(1))*u(1); 
                
                [S_a,Si_a,C_a,Ci_a,Lam_a]=obj.prob.eulerJac(rho(1), u(1),c(1));                
         ...
         function [eflax, deflax]=eulerflux_b(obj, w)
                 ...
                 e(2)= P(end)/(obj.prob.gamma-1)+rho(end)*u(end)^2/2;
                 c(2) = sqrt(obj.prob.gamma*P(2)/rho(2)); %Speed sound at boundary b
                 flax_b(1,1) = rho(2)*u(2); flax_b(2,1)=rho(2)*u(2)*u(2)+P(2); 
                 flax_b(3,1) = (e(2)+P(2))*u(2);
                 
                 [S_b,Si_b,C_b,Ci_b,Lam_b]=obj.prob.eulerJac(rho(2), u(2),c(2));
