classdef ROM < handle
    ...
    properties (SetAccess = private, GetAccess = public)
        ...
        numExecute=0;
        newtonSolver = 3; % 1 = original, 2 = constrained least-squares 3= Zimmerman's method
    end
...
%                 end
        ...
        function  [] = NewtonRaphsonZim(obj)
            ...
            for k=1:500%obj.newt.maxIter
                ...
                disp(['norm of constraints is ',num2str(norm(g(:,:)))])
								%kevin=load('kevin');
								%kevinw_guess = obj.phi' * (kevin.fom.sv(:,2) -kevin.fom.sv(:,1));
                %keving(1,:)=obj.constr1(kevinw_guess);
                %keving(2,:)=obj.constr2(kevinw_guess);
                %keving(3,:)=obj.constr3(kevinw_guess);
                %disp(['norm of kevin constraints is ',num2str(norm(keving(:,:)))])
                %keyboard
                Dg=[ vol*Phi1+dF_b(1,:)*Psi_b-dF_a(1,:)*Psi_a;
                          vol*Phi2+dF_b(2,:)*Psi_b-dF_a(2,:)*Psi_a;
                          vol*Phi3+dF_b(3,:)*Psi_b-dF_a(3,:)*Psi_a ];  
                ...
                U = reshape(obj.sv(:,itnump1),3,obj.prob.nVol);
                ...
                e   = [P(1)/(obj.prob.gamma-1)+rho(1)*u(1)^2/2,U(3,2:end-1),P(end)/(obj.prob.gamma-1)+rho(end)*u(end)^2/2]; %Energy
