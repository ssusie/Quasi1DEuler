...
%               keyboard
		...
		if i_N==1, out=gradientcheck( @(w) obj.CONS1(w), w_guess, params)
keyboard
		end
                H=Df'*Df; h=Df'*Rhat;
                P=H\Dg';
...
%               keyboard
                derQ=PhiQ2*pinv(PhiQ2(obj.sampleNodes,:))*squeeze(dQ(1,:,:))'; %nVol x 3 matrix, 3 corresponds to derivative wrt rho, rho*u and e                
                derivQ=zeros(1, length(w_increment));
...
%                    keyboard
                    ...
                    DerivConstr=[(obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi1(2:end-1,:);
                        (obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi2(2:end-1,:);
                        (obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi3(2:end-1,:) ]+...
                        obj.time.dt*(JL*obj.problem.phi + JR*obj.problem.phi- [zeros(1,length(w_increment));derivQ;zeros(1,length(w_increment))]);
...
%                    Jright=obj.problem.phiFrnew*pinv(obj.problem.phiFrnew(setdiff(obj.sampleInd,[1,2,3])-3,:))*JRnew;
                    ...
                    DerivConstr=[(obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi1(2:end-1,:); % NO SOL DEPENDENCE

                        (obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi2(2:end-1,:); % NO SOL DEPENDENCE
...
function []=myroe(obj, w_increment)
