classdef GNAT < handle
    ...
    properties (SetAccess = private, GetAccess = public)
        ...
        nSampleProd;
        addInd;
        solver=3; % =1 original, =2 Rom constrains; =3 Gnat constraints
        constr;
        Cnorm;
        reconFright;
        reconFright1;
...
%            obj.phiYhat = romobj.phi(unique(obj.jrow),:);
        ...
        function  [] = executeModel(obj)
            ...
            obj.reconFright=obj.problem.phiFright*pinv(obj.problem.phiFright(setdiff(obj.sampleInd,[1,2,3])-3,:));
            obj.reconFleft =obj.problem.phiFleft *pinv(obj.problem.phiFleft(setdiff(obj.sampleInd,[1,2,3])-3,:));
...
function  [] = RomConstraints(obj)
        ...
        for i_N = 1:20 %loop until the maximum newton iterations are reached
            ...
            if obj.problem.ncell==1
                %[g,Dg]=obj.constraintsApproxConstr(w_guess);
                [g,Dg]=obj.problem.constraintsForGNAT(w_guess, obj.fullSV(:, itnump1), obj.time.dt);
                %[g_approx,Dg_approx]=obj.constraintsApproxConstr(w_guess);
                %keyboard
                %[gReal,DgReal]=obj.problem.constraintsOneDomain(w_guess, obj.fullSV(:, itnump1), obj.time.dt);
                %               obj.approx_realConstrNorm=norm(g-gReal)

                %                       disp(['norm approximated - real constraints   ', num2str(norm(g-gReal))])
            else
        ...
        %       disp(['number of iterations   ', num2str(i_N)])
        %[gApprox,~]=obj.constraintsApproxConstr(w_guess);
        ...
        %disp(['norm of the approx constraint   ', num2str(norm(gApprox))])

        %obj.aconstr=[obj.aconstr,gApprox];
        obj.Cnorm=[obj.Cnorm,norm(gReal)];
        %Aconstr=obj.aconstr;
        %Rconstr=obj.rconstr;
        %save GnatReal_approxConstr99 Aconstr
...
% %                 keyboard
%                 for ii=1:3
%                     out=gradientcheck( @(w) obj.testconstrApproxConstr(w, ii), w_guess, params);
...
%                 end
 ...
 function  [Constr, DerivConstr] = constraintsApproxConstr(obj, w_increment)
         % keyboard
...
%Susie          %check if reconstruction is good      
                %keyboard
                %load Fright
                %norm(Fright(:,1)-fluxRight)  %=1.4842e-08
...
%             keyboard
            ...
            keyboard
%             PhiQ2=obj.problem.phiQ(2:3:end,:);
%             reconQ2=PhiQ2*pinv(PhiQ2(obj.sampleNodes(2:end),:))*Q(2,2:end)';
...
%          [f,df]=obj.myLeftFlux3(w_increment);
...
%          [f,df]=obj.myRightFlux3(w_increment);
...
function [c,dc]=testconstrApproxConstr(obj,w_increment, ind)
    [C,DC]=obj.constraintsApproxConstr(w_increment);
    c=C(ind,:); dc=DC(ind,:)';
end            
