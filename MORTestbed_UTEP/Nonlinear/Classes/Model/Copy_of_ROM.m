classdef ROM < handle
	...
	LocBasisHist;
        ...
        numExecute=0;
        newtonSolver = 2; % 1 = original,  2= Zimmerman's method;  3=  more constraints
    end
    ...
    methods
        %Constructor
        function [obj] = ROM(ROMfile,romtype,romid,probobj, cellnum, basNum)
...
% 		keyboard 
             ...
             g(1,:)=obj.constr1(w_guess, obj.trunc);
             g(2,:)=obj.constr2(w_guess, obj.trunc);
             g(3,:)=obj.constr3(w_guess, obj.trunc);
             
             obj.gOrig=[obj.gOrig g];
             Corig=obj.gOrig;
             save PGconstr999 Corig
...
%         function  [] = NewtonRaphsonConstrained(obj)
%             
%             itnump1 = obj.cTimeIter + 1;
%             obj.sv(:,itnump1)=obj.sv(:,obj.cTimeIter);
%             gamma=obj.prob.gamma;
%            
...
%                         
%             vol=(obj.prob.mesh.gridpt(2:end,1)-obj.prob.mesh.gridpt(1:end-1,1))'; %row vector
%             
%             Psi_a= [Phi1(1,:);Phi2(1,:); Phi3(1,:)];
%             Psi_b= [Phi1(end,:);Phi2(end,:); Phi3(end,:)];
%             
%             t = obj.time.T(1) + obj.time.dt*obj.cTimeIter; 
%             [Res,Jres]=obj.TimeScheme.TimeIntNLFunc(obj.prob,obj.sv(:,itnump1),obj.sv(:,itnump1),t);
%             Jres=Jres*obj.phi;
%             Hres=Jres.'*Jres;
%             
%             w_guess = obj.phi'* (obj.sv(:,itnump1)-obj.sv(:,obj.cTimeIter)); %initial guess for generalized coordinates for Newton iteration
%             l_guess = [1;1;1]; % guess for lambda in optimization problem for Newton interation
%             
%             if obj.printLevel > 1 
...
%                 [~,dQ]=obj.prob.forceTerm(u,P);
%                 [~, droeF]=obj.prob.roeFlux(rho,u,P,c,e,dc);
%                
%                 constr(1,:)=obj.constr1(w_guess);
%                 constr(2,:)=obj.constr2(w_guess);
%                 constr(3,:)=obj.constr3(w_guess);
%                 derivQ=zeros(3,5);
...
% 
%                 J2L = [-obj.prob.S(2)*droeF(:,1:3,1),- obj.prob.S(2)*droeF(:,4:6,1)];
%                 J2R = [obj.prob.S(obj.prob.nVol)*droeF(:,1:3,obj.prob.nVol-1),obj.prob.S(obj.prob.nVol)*droeF(:,4:6,obj.prob.nVol-1)];
%                 Phi=obj.phi;
%                Jconstr= [(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi1(2:end-1,:); (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi2(2:end-1,:); (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi3(2:end-1,:)]+...
%                      obj.time.dt*(J2L(:,1:3)*Phi(1:3,:) +J2L(:,4:6)*Phi(4:6,:)+ ...
%                                   J2R(:,1:3)*Phi(end-5:end-3,:)+ J2R(:,4:6)*Phi(end-2:end,:)- derivQ);                                                        
%                 
%                 H1=hessian(@obj.constr1,w_guess);
%                 H2=hessian(@obj.constr2,w_guess);
%                 H3=hessian(@obj.constr3,w_guess);
% %                                               
% %                 H1=Jconstr(1,:).'*Jconstr(1,:);
% %                 H2=Jconstr(2,:).'*Jconstr(2,:);
% %                 H3=Jconstr(3,:).'*Jconstr(3,:);
% 
% %                 H = [Hres+l_guess(1)*H1+l_guess(2)*H2+l_guess(3)*H3, Jconstr.'; Jconstr, zeros(3)];               
% %                 h = [Jres.'*Res+Jconstr.'*l_guess; constr];
% %                 delta=H\h;
% 
%                 A11=Hres+l_guess(1)*H1+l_guess(2)*H2+l_guess(3)*H3; A12=Jconstr.';
%                 A21=Jconstr; A22=zeros(3);
%                 b1=Jres.'*Res+Jconstr.'*l_guess;
%                 b2=constr;
%                 
%                 M2 = A21 * (A11\A12);
%                 r2 = b2 - A21 * (A11\b1);
%                 del_l = M2\r2;
%                 del_w = A11\(b1 - A12 * del_l);
%                 
% %                 if obj.newt.linesrch.status
% %                     %alpha = linesearch(@(alpha) obj.TimeScheme.TimeIntNLFuncDir(obj.prob,obj.sv(:,itnump1-indexAdj),obj.sv(:,itnump1-1),t,alpha,du,obj),obj.newt.linesrch.prop);                
% %                     alpha = linesrchBackNewton(@(alpha) obj.TimeScheme.TimeIntNLFuncDir(obj.prob,obj.sv(:,itnump1-indexAdj),obj.sv(:,itnump1-1),t,alpha,del_w,obj));
% %                     del_w = alpha*del_w;
% %                 end
% 
%                 w_guess=w_guess+del_w;
%                 l_guess=l_guess+del_l;
%                 
%           
%                 obj.sv(:,itnump1)=obj.sv(:,obj.cTimeIter)+obj.phi*w_guess;
%                 
%                 if norm([del_w; del_l],2)<10^(-5)
%                     break;
%                     
%                 end
%                 
...
%             end
%             
%             disp(['My constrained: number of iterations ',num2str(k)])
%         end
%         
%         
        function  [] = NewtonRaphsonZim(obj)
...
%  				keyboard
%                 [R1,J1,Cpp,dCpp]=obj.prob.fullImplicitInletBC(rho(1:2),u(1:2),P(1:2),c(1:2),dc_p(1:2,:));
%                 [R3,J3,Cp,dCp]=obj.prob.fullImplicitOutletBC(rho(end-1:end),u(end-1:end),P(end-1:end),c(end-1:end),dc_p(end-1:end,:));
%                 
                g(1:3,:)=[(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi1(2:end-1,:)*w_guess; 
                          (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi2(2:end-1,:)*w_guess; 
                          (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi3(2:end-1,:)*w_guess ]+...
                          obj.time.dt*(Rright(:,end)+Rleft(:,1)-Q(:,2:end-1)*(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))');
%                     keyboard     
                      % fullImplicit...BC
%                 g(4:6,:)=Cpp*(obj.phi(1:3,:)*w_guess)/obj.time.dt+R1;
%                 g(7:9,:)=Cp*(obj.phi(end-2:end,:)*w_guess)/obj.time.dt+R3;
                
%                 disp(['norm of constraints is ',num2str(norm(g(:,:)))])

%                  keyboard
%                  [obj.constr1(w_guess,5);obj.constr2(w_guess,5);obj.constr3(w_guess,5)]
...
%               
                ...
                Dg= [(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi1(2:end-1,:); (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi2(2:end-1,:); (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi3(2:end-1,:)]+...
                     obj.time.dt*(J2L(:,1:3)*obj.phi(1:3,:) +J2L(:,4:6)*obj.phi(4:6,:)+ ...
                                  J2R(:,1:3)*obj.phi(end-5:end-3,:)+ J2R(:,4:6)*obj.phi(end-2:end,:)- derivQ);
        ...
        function  [] = NewtonRaphsonZim_trunc(obj)
           ...
           Phi=  obj.phi(:,1:obj.trunc);
            ...
            for k=1:20 %obj.newt.maxIter
                ...
                if obj.cTimeIter==1 & k==1
                    %size(Phi)
                    disp(['Zimmerman truncate, number of cells  ', num2str(obj.ncell)])
                end
                [Res,Jres]=obj.TimeScheme.TimeIntNLFunc(obj.prob,obj.sv(:,itnump1),obj.sv(:,itnump1-1),t);
                Df=Jres*Phi;
		[g,Dg]=obj.oneCellconstraints(w_guess); 
%                 keyboard
                H=Df'*Df; h=Df'*Res;
                P=H\Dg';
                x=H\h;
...
%             w_guess=Phi'*(obj.sv(:,itnump1)-obj.sv(:,obj.cTimeIter));   
             ...
             obj.gZim=[obj.gZim g];
             Czim=obj.gZim;
             save constrZim9999 Czim


             disp(['number of iterations in Zim_trunc  ',num2str(k)])
        end
        
function [constr, dconstr]=oneCellconstraints(obj, w_increment)
...
%                 [R3,J3,Cp,dCp]=obj.prob.fullImplicitOutletBC(rho(end-1:end),u(end-1:end),P(end-1:end),c(end-1:end),dc_p(end-1:end,:));
%                 
                constr(1:3,:)=[(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi1(2:end-1,:)*w_increment; 
...
%                 disp(['norm of constraints is ',num2str(norm(g(:,:)))])
                ...
                for i=2:obj.prob.nVol-1%1:obj.prob.nVol
                   derivQ=derivQ+squeeze(dQ(:,:,i))*[Phi1(i,:);Phi2(i,:);Phi3(i,:)]*(obj.prob.SVol(i).*obj.prob.dx(i));                    
                end
%               
                %from governEqn 
...
%                 J2L = -obj.prob.S(2)*droeF(:,:,1); %[-obj.prob.S(2)*droeF(:,1:3,1),- obj.prob.S(2)*droeF(:,4:6,1)];
%                 J2R = obj.prob.S(obj.prob.nVol)*droeF(:,:,obj.prob.nVol-1); %[obj.prob.S(obj.prob.nVol)*droeF(:,1:3,obj.prob.nVol-1),obj.prob.S(obj.prob.nVol)*droeF(:,4:6,obj.prob.nVol-1)];
% 		
%                 Dg= [(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi1(2:end-1,:); (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi2(2:end-1,:); (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi3(2:end-1,:)]+...
%                      obj.time.dt*(J2L(:,1:3)*Phi(1:3,:) +J2L(:,4:6)*Phi(4:6,:)+ ...
%                                   J2R(:,1:3)*Phi(end-5:end-3,:)+ J2R(:,4:6)*Phi(end-2:end,:)- derivQ);
%                 
                ...
                %derivative at point x ( x corresponds to location start/finish)
                %keyboard
                start=1;
                ...
                dconstr= [(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi1(2:end-1,:); (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi2(2:end-1,:); (obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi3(2:end-1,:)]+...
...
function [c,dc]=oneCellc1(obj, w_increment)
	[C,DC]=obj.oneCellconstraints(w_increment);
	c=C(1,:);
	dc=DC(1,:)';
end
...
%                disp(['NR trunc'])
                ...
                obj.NewtonRaphsonZim_trunc();
%                 disp(['Zim trunc'])
            end
        else
	    if 3*obj.ncell<obj.trunc
		 obj.NewtonRaphsonZim_subdomains();
%                disp(['constriants on subdomains'])
            elseif 3*obj.ncell==obj.trunc
                obj.solveConstraints();
%                disp(['solve only constriants'])
            elseif 3*obj.ncell>obj.trunc
                obj.minimizeConstraints();
%                disp([' minimize constriants'])
            end           
        end
    end
    
    
 function  [] = NewtonRaphsonZim_subdomains(obj)
	...
	%    end
            ...
            for k=1:20 %obj.newt.maxIter
                ...
                Df=Jres*Phi;
	        %keyboard    
                [g,Dg]=obj.constraints(w_guess);
                %out=gradientcheck( @(w) obj.CONS1(w), w_guess)
                H=Df'*Df; h=Df'*Res;
                P=H\Dg';
                x=H\h;
                S=-inv(Dg*P);
            ...
            %w_guess=Phi'*(obj.sv(:,itnump1)-obj.sv(:,obj.cTimeIter));   
            %g(1,:)=obj.constr1(w_guess, trunc);
            %g(2,:)=obj.constr2(w_guess, trunc);
            %g(3,:)=obj.constr3(w_guess, trunc);
        ...
        function c1=constraints1(obj, w_increment)
            [Cnst,~]=obj.constraints(w_increment);
            c1=Cnstr(1,:);
        end
        
        function c2=constraints2(obj, w_increment)
            [Cnst,~]=obj.constraints(w_increment);
            c2=Cnstr(2,:);
        end
        
        function c3=constraints3(obj, w_increment)
            [Cnst,~]=obj.constraints(w_increment);
            c3=Cnstr(3,:);
        end
        
        
	function [cnstr, Dcnstr]=constraints(obj, w_increment)
		%keyboard
...
%                 finish=size(roeF,2);
    ...
    function [c,dc]=CONS1(obj,w_increment)
        [C,DC]=obj.constraints(w_increment);
        c=C(1,:); dc=DC(1,:)';
    end

function [c,dc]=CONS2(obj,w_increment)
        [C,DC]=obj.constraints(w_increment);
        c=C(2,:); dc=DC(2,:)';
end
    
function [c,dc]=CONS3(obj,w_increment)
        [C,DC]=obj.constraints(w_increment);
        c=C(3,:); dc=DC(3,:)';
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
         ...
         function c=constr1(obj,w_increment, trunc)
             	Phi=obj.phi(:,1:trunc);
                obj.sv(:,obj.cTimeIter+1)=obj.sv(:,obj.cTimeIter)+Phi*w_increment;
            
                [rho, u, P,c,e,~,dc]=obj.prob.getVariables(obj.sv(:,obj.cTimeIter+1));
                [Q,~]=obj.prob.forceTerm(u,P);
	            ...
	            Rleft= -bsxfun(@times,roeF(:,1:end-1),obj.prob.S(2:end-2));
                Phi1=Phi(1:3:end,:);
                c=(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi1(2:end-1,:)*w_increment+...
                   obj.time.dt*(Rright(1,end)+Rleft(1,1)-Q(1,2:end-1)*(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))');
                       
         end
         
         function c=constr2(obj,w_increment, trunc)
	            ...
	            Rleft= -bsxfun(@times,roeF(:,1:end-1),obj.prob.S(2:end-2));
                Phi2=Phi(2:3:end,:);
                c=(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi2(2:end-1,:)*w_increment+...
                   obj.time.dt*(Rright(2,end)+Rleft(2,1)-Q(2,2:end-1)*(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))');
                       
             
         end
         
         function c=constr3(obj,w_increment, trunc)
 		...
 		Phi=obj.phi(:,1:trunc);            
	            ...
	            Rleft= -bsxfun(@times,roeF(:,1:end-1),obj.prob.S(2:end-2));
                Phi3=Phi(3:3:end,:);
                c=(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))*Phi3(2:end-1,:)*w_increment+...
                   obj.time.dt*(Rright(3,end)+Rleft(3,1)-Q(3,2:end-1)*(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1))');
...
%                         for jj=1:size(X,2)
%                             [rho, u, P,c,e,~,dc]=obj.prob.getVariables(X(:,jj));
%                             [Q,dQ]=obj.prob.forceTerm(u,P);
%                             [roeF, droeF]=obj.prob.roeFlux(rho,u,P,c,e,dc);
%                             
%                             % governEqn
...
% 
% 			%figure(1)
% 			%plot(cumsum(diag(sr))/sum(diag(sr)),'ko')
% 			%figure(2)
% 			%plot(cumsum(diag(sl))/sum(diag(sl)),'ko')
% 			%figure(3)
% 			%plot(cumsum(diag(sQ))/sum(diag(sQ)), 'ko')
% %             vec1=cumsum(diag(sr))/sum(diag(sr));
% %             vec2=cumsum(diag(sQ))/sum(diag(sQ));
% 			mF=min(find(cumsum(diag(sr))/sum(diag(sr))>0.999));
% 			mQ=min(find(cumsum(diag(sQ))/sum(diag(sQ))>0.999));
% % 			keyboard	
% 			obj.phiFright=ul(:,1:mF); %99.9%
% 			obj.phiFleft=ur(:,1:mF);  %99.9%
% 			obj.phiQ=uQ(:,1:mQ);
            ...
            switch obj.nBases
                case 1
                    ...
                    for i_t = firstStep:nstep %Loop over each time step to determine all state vectors
                        ...
                        % elseif obj.newtonSolver == 2;
                        %     obj.NewtonRaphsonConstrained();
                        
                        %  elseif obj.newtonSolver == 3;
                        %      obj.NewtonRaphsonZim();
                        %  elseif obj.newtonSolver==4
                        
                        %     truncate=min(find(cumsum(obj.SingVals)/sum(obj.SingVals)>0.999));
                        
                        %obj.trunc=truncate;
                        %obj.NewtonRaphsonZim_trunc(truncate);
                        
                        % keyboard
                        %                              obj.NewtonRaphson_trunc(5);
                        %else
                        %    obj.ncell=3;
                        %truncate=min(find(cumsum(obj.SingVals)/sum(obj.SingVals)>0.999));
                        %                            obj.NewtonRaphsonZim_new(15, 2); %(trunc, ncell)
                        %end
                        
                        % 			obj.ncell=1; obj.trunc=obj.prob.nY;
                        
                        if i_t==nstep %firstStep
                            NewFrBasis=[];
                            NewFlBasis=[];
                            % load soln
			    soln=obj.sv(:,1:end-1);
                            ...
                            save forceQ forceQ
                            %keyboard
                            [ur2,sr2,~]=svd(NewFrBasis);
                            [ul2,sl2,~]=svd(NewFlBasis);
                            [ur,sr,~]=svd(Fright);
                            [ul, sl, ~]=svd(Fleft);
                            [uQ,sQ,~]= svd(forceQ);
                            [uroe1,s1,~]=svd(roeF1);
