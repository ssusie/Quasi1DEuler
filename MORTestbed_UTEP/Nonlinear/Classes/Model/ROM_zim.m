...
%                 end
        ...
        function [eflax, deflax]=eulerflux_a(obj, w)
            
                U=reshape(obj.sv(:,obj.cTimeIter + 1)+obj.phi*w,3,obj.prob.nVol); 
         ...
         function [eflax, deflax]=eulerflux_b(obj, w)
                 U=reshape(obj.sv(:,obj.cTimeIter + 1)+obj.phi*w,3,obj.prob.nVol); 
         ...
         function c=constr1(obj,w)
             
             [flxGuess_a,~] = obj.eulerflux_a(w); 
             [flxGuess_b,~] = obj.eulerflux_b(w);
             vol=(obj.prob.mesh.gridpt(2:end,1)-obj.prob.mesh.gridpt(1:end-1,1))'; %row vector
             Phi1=obj.phi(1:3:end,:); %basis for the first conserved quantity rho
             Phi2=obj.phi(2:3:end,:); %basis for the second conserved quantity rho*u
             Phi3=obj.phi(3:3:end,:);
             w0 = obj.phi'* obj.sv(:,obj.cTimeIter); %solution at previous time step
             [flx_a, ~]=obj.eulerflux_a(w0); 
             [flx_b, ~]=obj.eulerflux_b(w0);
             
             c=vol*Phi1*(w-w0)-obj.time.dt/2*(flxGuess_a(1)+flx_a(1)-flxGuess_b(1)-flx_b(1));
             
         end
         
         function c=constr2(obj,w)
             ...
             c=vol*Phi2*(w-w0)-obj.time.dt/2*(flxGuess_a(2)+flx_a(2)-flxGuess_b(2)-flx_b(2));
             
         end
         
         function c=constr3(obj,w)
             ...
             c=vol*Phi3*(w-w0)-obj.time.dt/2*(flxGuess_a(3)+flx_a(3)-flxGuess_b(3)-flx_b(3));
