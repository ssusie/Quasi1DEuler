...
%               keyboard
            for i_N = 1:10 %loop until the maximum newton iterations are reached
		...
		%disp('kevin')
                ...
                %[g,Dg]=obj.constraintsGNAT(w_guess);
                %[g,Dg]=obj.constraints_old(w_guess);
		[g,Dg]=obj.constraints_oldnew(w_guess);
                %if obj.cTimeIter==2 || obj.cTimeIter==5
%                keyboard
                %end
		...
		keyboard
		if i_N==1 
			out1=gradientcheck( @(w) obj.CONS1(w), w_guess, params)
			out2=gradientcheck( @(w) obj.CONS2(w), w_guess, params)
			out3=gradientcheck( @(w) obj.CONS3(w), w_guess, params)
			keyboard
		end
                H=Df'*Df; h=Df'*Rhat;
                P=H\Dg';
...
function [Constr, DerivConstr]=constraintsGNAT(obj, w_increment)
...
%               keyboard
                derQ=PhiQ2*pinv(PhiQ2(obj.sampleNodes,:))*squeeze(dQ(1,:,:))'; %nVol x 3 matrix, 3 corresponds to derivative wrt rho, rho*u and e                
                sumDerivQ=zeros(1, length(w_increment));
                
                for i=2:obj.problem.prob.nVol-1
                    sumDerivQ=sumDerivQ+derQ(i,:)*[Phi1(i,:);Phi2(i,:);Phi3(i,:)]*(obj.problem.prob.SVol(i).*obj.problem.prob.dx(i));
                end
...
%                    keyboard
                    ...
                    DerivConstr=[(obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi1(2:end-1,:);
                        (obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi2(2:end-1,:);
                        (obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi3(2:end-1,:) ]+...
                        obj.time.dt*(JL*obj.problem.phi + JR*obj.problem.phi- [zeros(1,length(w_increment));sumDerivQ;zeros(1,length(w_increment))]);
                 
                end 
end   


        function  [Constr, DerivConstr] = constraints_old(obj, w_increment)
...
%               keyboard 
                if obj.probGNAT.ind1
                    ...
                    %fluxLeft = obj.problem.phiFlnew *pinv(obj.problem.phiFlnew(setdiff(obj.sampleInd,[1,2,3])-3,:))*Rleft(:);
                    %fluxRight= obj.problem.phiFrnew*pinv(obj.problem.phiFrnew(setdiff(obj.sampleInd,[1,2,3])-3,:))*Rright(:);
...
%                    Jleft =obj.problem.phiFlnew*pinv(obj.problem.phiFlnew(setdiff(obj.sampleInd,[1,2,3])-3,:))*JLnew;
%                    Jright=obj.problem.phiFrnew*pinv(obj.problem.phiFrnew(setdiff(obj.sampleInd,[1,2,3])-3,:))*JRnew;
                    ...
                    DerivConstr=[(obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi1(2:end-1,:);
                        (obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi2(2:end-1,:);
                        (obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi3(2:end-1,:) ]+...
                        obj.time.dt*(Jleft(1:3,:)*obj.problem.phi + ...
                               Jright(end-2:end,:)*obj.problem.phi- ...
                           [zeros(1,length(w_increment));sumDerivQ;zeros(1,length(w_increment))]);
                    
%dfr= (Jright(end,:)*obj.problem.phi)';
            end

        end


 function  [Constr, DerivConstr] = constraints_oldnew(obj, w_increment)
                    % keyboard 
                Phi1=obj.problem.phi(1:3:end,:); %basis for the first conserved quantity rho
                Phi2=obj.problem.phi(2:3:end,:); %basis for the second conserved quantity rho*u
                Phi3=obj.problem.phi(3:3:end,:); %basis for the thirs conserved quantity e
               ...
               keyboard 
                if obj.probGNAT.ind1
			[fluxLeft, Jleft, sq, dsq] = obj.myLeftFlux3QdQ(w_increment);
			[fluxRight,Jright]=obj.myRightFlux(w_increment);
...
function [l, dl, sQ,sdQ]=myLeftFlux3QdQ(obj, w_increment)
		...
		sQ= [(obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*PhiQ1(2:end-1,:)*pinv(PhiQ1(obj.sampleNodes,:))*Q(1,:)';
                        (obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*PhiQ2(2:end-1,:)*pinv(PhiQ2(obj.sampleNodes,:))*Q(2,:)';
                        (obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*PhiQ3(2:end-1,:)*pinv(PhiQ3(obj.sampleNodes,:))*Q(3,:)'];
...
function  [fr, dfr] =myRightFlux3(obj, w_increment)
		...
		Phi1=obj.problem.phi(1:3:end,:); %basis for the first conserved quantity rho
		 ...
		 Jright=Jright';
                    ...
                    %dfr= (Jright(end,end-5:end)*obj.problem.phi(end-5:end,:))';
                    dfr= (Jright(end-2:end,:)*obj.problem.phi);
%keyboard
...
function [l, dl]=myLeftFlux(obj, w_increment)
...
function  [fr, dfr] =myRightFlux(obj, w_increment)
...
%         [C,DC]=obj.constraintsGNAT(w_increment);
	 [C,DC]=obj.constraints_old(w_increment);
         c=C(1,:); dc=DC(1,:)';
    end
