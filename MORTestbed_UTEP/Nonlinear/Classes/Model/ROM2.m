classdef ROM < handle
    ...
    properties (SetAccess = private, GetAccess = public)
        ...
        numExecute=0;
        newtonSolver = 2; % 1 = original, 2 = constrained least-squares 3= Zimmerman's method
    end
    ...
    methods
        %Constructor
        function [obj] = ROM(ROMfile,romtype,romid,probobj)
...
%                 end
        ...
        function  [] = NewtonRaphsonZim(obj)
            ...
            for k=1:20 %obj.newt.maxIter
                ...
                % governEqn
                %Rright= bsxfun(@rdivide, bsxfun(@times,roeF(:,2:end),obj.prob.S(3:end-1)),obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1));
		        %Rleft= -bsxfun(@rdivide, bsxfun(@times,roeF(:,1:end-1),obj.prob.S(2:end-2)),(obj.prob.SVol(2:end-1).*obj.prob.dx(2:end-1)));
...
%                 J2L = [-(1./(obj.prob.SVol(2)*obj.prob.dx(2)))*obj.prob.S(2)*droeF(:,1:3,1),...
%                     (1./(obj.prob.SVol(2)*obj.prob.dx(2)))*(- obj.prob.S(k)*droeF(:,4:6,1))];
%                 J2R = [1./(obj.prob.SVol(obj.prob.nVol-1)*obj.prob.dx(obj.prob.nVol-1))*(obj.prob.S(obj.prob.nVol)*droeF(:,1:3,obj.prob.nVol-1)),... 
%                     (1./(obj.prob.SVol(obj.prob.nVol-1)*obj.prob.dx(obj.prob.nVol)))*obj.prob.S(obj.prob.nVol)*droeF(:,4:6,obj.prob.nVol-1) ];
...
%                 D1=Cpp*obj.phi(1:3,:)+...
%                      [squeeze(dCpp(:,:,1))*(obj.phi(1:3,:)*w_guess)*Phi1(1,:)]+ [squeeze(dCpp(:,:,2))*(obj.phi(1:3,:)*w_guess)*Phi2(1,:)]+[squeeze(dCpp(:,:,3))*(obj.phi(1:3,:)*w_guess)*Phi3(1,:)]+...
%                      obj.time.dt*([J1(1,:)*(obj.phi(1:6,:)*w_guess)*Phi1(1,:);J1(2,:)*(obj.phi(1:6,:)*w_guess)*Phi2(1,:);J1(3,:)*(obj.phi(1:6,:)*w_guess)*Phi3(1,:)]);
% %                      obj.time.dt*obj.phi(1:3,:);                 
%                 D3=Cp*obj.phi(end-2:end,:)+...
%                      (squeeze(dCp(:,:,1))*(obj.phi(end-2:end,:)*w_guess)*Phi1(end,:)+squeeze(dCp(:,:,2))*(obj.phi(end-2:end,:)*w_guess)*Phi2(end,:)+squeeze(dCp(:,:,3))*(obj.phi(end-2:end,:)*w_guess)*Phi3(end,:))+...
%                      obj.time.dt*([J3(1,:)*(obj.phi(end-5:end,:)*w_guess)*Phi1(end,:);J3(2,:)*(obj.phi(end-5:end,:)*w_guess)*Phi2(end,:);J3(3,:)*(obj.phi(end-5:end,:)*w_guess)*Phi3(end,:)]);
% %                      obj.time.dt*obj.phi(end-2:end,:);
...
%                 if obj.newt.linesrch.status
%                     %alpha = linesearch(@(alpha) obj.TimeScheme.TimeIntNLFuncDir(obj.prob,obj.sv(:,itnump1-indexAdj),obj.sv(:,itnump1-1),t,alpha,du,obj),obj.newt.linesrch.prop);                
%                     alpha = linesrchBackNewton(@(alpha) obj.TimeScheme.TimeIntNLFuncDir(obj.prob,obj.sv(:,itnump1),obj.sv(:,itnump1-1),t,alpha,del_w,obj))
%                     del_w = alpha*del_w;
%                 end

                w_guess=w_guess+del_w;
                  
%                 if length(find(rho<0))>0 || length(find(c<0))>0 || length(find(P<0))>0
%                     fprintf('negative\n')
%                     %break
%                     
%                 end
        ...
        function [eflax, deflax]=eulerflux_a(obj, w_increment)
            
                U=reshape(obj.sv(:,obj.cTimeIter)+obj.phi*w_increment,3,obj.prob.nVol); 
                rho=U(1,1); u=U(2,1); P=U(3,1);               
                e = P/(obj.prob.gamma-1)+rho*u^2/2;                
                c = sqrt(obj.prob.gamma*P/rho); %Speed of sound at boundary a                
                flax_a(1,1) = rho*u; flax_a(2,1)=rho*u*u+P; 
                flax_a(3,1) = (e+P)*u; 
                
                [S_a,Si_a,C_a,Ci_a,Lam_a]=obj.prob.eulerJac(rho,u,c);                
                dF_a=Si_a*Ci_a*Lam_a*C_a*S_a;                               
                eflax=flax_a;                
                deflax=dF_a;
        end
                
                
         function [eflax, deflax]=eulerflux_b(obj, w_increment)
                 U=reshape(obj.sv(:,obj.cTimeIter)+obj.phi*w_increment,3,obj.prob.nVol); 
                 rho=U(1,end); u=U(2,end); P=U(3,end);
                 e = P/(obj.prob.gamma-1)+rho*u^2/2;
                 c = sqrt(obj.prob.gamma*P/rho); %Speed sound at boundary b
                 flax_b(1,1) = rho*u; flax_b(2,1)=rho*u*u+P; 
                 flax_b(3,1) = (e+P)*u;
                 
                 [S_b,Si_b,C_b,Ci_b,Lam_b]=obj.prob.eulerJac(rho,u,c);
                 dF_b=Si_b*Ci_b*Lam_b*C_b*S_b;
                 eflax=flax_b;
                 deflax=dF_b;
         end

         function c=constr1(obj,w_increment)
             
             [flxGuess_a,~] = obj.eulerflux_a(w_increment); 
             [flxGuess_b,~] = obj.eulerflux_b(w_increment);
             vol=(obj.prob.mesh.gridpt(2:end,1)-obj.prob.mesh.gridpt(1:end-1,1))'; %row vector
             Phi1=obj.phi(1:3:end,:); %basis for the first conserved quantity rho
             Phi2=obj.phi(2:3:end,:); %basis for the second conserved quantity rho*u
             Phi3=obj.phi(3:3:end,:);
             %w0 = obj.phi'* (obj.sv(:,obj.cTimeIter); %solution at previous time step
             [flx_a, ~]=obj.eulerflux_a(zeros(size(w_increment))); 
             [flx_b, ~]=obj.eulerflux_b(zeros(size(w_increment)));
             
						 keyboard
             %c=vol*Phi1*(w_increment)-obj.time.dt/2*(flxGuess_a(1)+flx_a(1)-flxGuess_b(1)-flx_b(1));
             c=vol*Phi1*(w_increment)-obj.time.dt*(flxGuess_a(1)-flxGuess_b(1));
             
         end
         
         function c=constr2(obj,w_increment)
             ...
             %c=vol*Phi2*(w_increment)-obj.time.dt/2*(flxGuess_a(2)+flx_a(2)-flxGuess_b(2)-flx_b(2));
             c=vol*Phi2*(w_increment)-obj.time.dt*(flxGuess_a(2)-flxGuess_b(2));
             
         end
         
         function c=constr3(obj,w_increment)
             ...
             %c=vol*Phi3*(w_increment)-obj.time.dt/2*(flxGuess_a(3)+flx_a(3)-flxGuess_b(3)-flx_b(3));
             c=vol*Phi3*(w_increment)-obj.time.dt*(flxGuess_a(3)-flxGuess_b(3));
             
         end


%	function c=constr4(obj, w_increment)
%		obj.sv(:, obj.cTimeIter)
%		[R1]=obj.prob.fullImplicitInletBC(rho, u, p,c,dc)
%		Res=obj.phi(1:3,:)*w_increment+obj.time.dt*R1;
%		c=Res;
%	end
