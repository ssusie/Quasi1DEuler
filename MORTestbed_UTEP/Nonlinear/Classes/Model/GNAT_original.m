classdef GNAT < handle
    ...
    properties (SetAccess = private, GetAccess = public)
        ...
        nSampleProd;
        addInd;
        solver=1; % =1 original, =2 constrains
...
%             keyboard
		...
		FR=reshape(bsxfun(@times,roeF(:,obj.probGNAT.iarrayFaceI+1),obj.probGNAT.S(obj.probGNAT.iarrayFace(1+obj.probGNAT.ind1:end-obj.probGNAT.indN)+1)), 3*length(obj.probGNAT.iarrayFaceI+1),1);
...
%                 keyboard
                ...
                if obj.probGNAT.ind1
                    ...
                    g(1:3,:)=[(obj.problem.prob.SVol(2:end-1).*obj.problem.prob.dx(2:end-1))*Phi1(2:end-1,:)*w_guess;
                        ...
                        obj.time.dt*([PhiFr1(end,:)*pinv(PhiFr1(setdiff(obj.sampleNodes,obj.probGNAT.ind1),:))*Rright(1,:)';
                        PhiFr2(end,:)*pinv(PhiFr2(setdiff(obj.sampleNodes,obj.probGNAT.ind1),:))*Rright(2,:)'; %%%%%%%%%%%%%%%%%%%%%%%%% wrong
                        PhiFr3(end,:)*pinv(PhiFr3(setdiff(obj.sampleNodes,obj.probGNAT.ind1),:))*Rright(3,:)']+ Rleft(:,1)-...
