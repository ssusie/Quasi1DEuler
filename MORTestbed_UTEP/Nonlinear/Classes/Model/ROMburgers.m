classdef ROM < handle
	...
	LocBasisHist;
        ...
        numExecute=0;
        newtonSolver = 1; % 1 = original,  2= Zimmerman's method;  3=  more constraints
    end
...
%              save PGconstr999 Corig
        ...
        function  [] = NewtonRaphsonZim_trunc(obj)
            ...
            for k=1:20 %obj.newt.maxIter
                ...
                Df=Jres*Phi;
%                 keyboard
                Fright=0.5*obj.sv(2:end, itnump1).^2;
                Fleft=-0.5*obj.sv(1:end-1,itnump1).^2;
                
                g= obj.prob.dx(2:end)'*Phi(2:end,:)*w_guess + ...
                          obj.time.dt*(Fright(end)+Fleft(1)-obj.prob.dx(2:end)'*obj.prob.G(2:end));
%                  g= obj.prob.dx'*Phi*w_guess+...
%                           obj.time.dt*(Fright(end)+Fleft(end)-obj.prob.dx(2:end)'*obj.prob.G(2:end));
%                

                disp(['norm of constraints is ',num2str(norm(g(:,:)))])
                N = obj.prob.config.ndof;
                JR = spdiags([ zeros(N,1), (1./obj.prob.dx).*obj.sv(:,itnump1)],[-1,0],N,N); % first line is BC
                JL = spdiags([-(1./[obj.prob.dx(2:end);obj.prob.dx(end)]).*obj.sv(:,itnump1), zeros(N,1)],[-1,0],N,N); %first line is BC
                
                Dg= obj.prob.dx(2:end)'*Phi(2:end,:)+ obj.time.dt*(JR(end,end)*Phi(end,:) + JL(2,1)*Phi(2,:));
...
%                     obj.time.dt*(right(:,points(2)-2)+left(:,1)-Q(:,current_points)*(obj.prob.SVol(current_points).*obj.prob.dx(current_points))');
%                 
...
%                         cnstr(3*(kk-1)+1:3*kk,:)=[(obj.prob.SVol(points(kk):points(kk+1)-1).*obj.prob.dx(points(kk):points(kk+1)-1))*Phi1(points(kk):points(kk+1)-1,:)*w_increment;
%                             (obj.prob.SVol(points(kk):points(kk+1)-1).*obj.prob.dx(points(kk):points(kk+1)-1))*Phi2(points(kk):points(kk+1)-1,:)*w_increment;
%                             (obj.prob.SVol(points(kk):points(kk+1)-1).*obj.prob.dx(points(kk):points(kk+1)-1))*Phi3(points(kk):points(kk+1)-1,:)*w_increment]+...
...
%                 
%                 %keyboard
%                 cnstr((obj.ncell-1)*3+1:obj.ncell*3,:)=[(obj.prob.SVol(points(end-1):end-1).*obj.prob.dx(points(end-1):end-1))*Phi1(points(end-1):end-1,:)*w_increment;
%                     (obj.prob.SVol(points(end-1):end-1).*obj.prob.dx(points(end-1):end-1))*Phi2(points(end-1):end-1,:)*w_increment;
%                     (obj.prob.SVol(points(end-1):end-1).*obj.prob.dx(points(end-1):end-1))*Phi3(points(end-1):end-1,:)*w_increment ]+...
