classdef SteadyTime < handle
    ...
    methods
        ...
        %Entire functions
        function  [R,J,iprev] = TimeIntNLFunc(obj,prob,U,~,t,precomp)
        ...
        function  [phi,dphi] = TimeIntNLFuncDir(obj,prob,U,~,t,alpha,v,rom)
            ...
            switch lower(obj.modelType(1))
                ...
                case 'p'
                    ...
                    phi = 0.5*norm(R)^2;
                    if sum(isnan(R))>0 || ~isreal(R) || ~isreal(J)
                        phi=inf;
                    end
                    dphi = v'*(rom.phi'*(J'*R));
%                     
