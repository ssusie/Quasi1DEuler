...
h_legend = legend([hfom,hrom1,hrom2,hrom3,hgnat0,hgnat1,hgnat2,hgnat3,hgnat4,hgnat5,hgnat6],METHODS);
set(h_legend,'FontSize',12)
xlabel('time step','FontSize',15)
ylabel('norm of method specific constraints','FontSize',15)
...
xlabel('time step','FontSize',15)
ylabel('rel error with FOM solution','FontSize',15)
h_legend = legend([hrom1,hrom2,hrom3,hgnat0,hgnat1,hgnat2,hgnat3,hgnat4,hgnat5,hgnat6],list);
set(h_legend,'FontSize',12)
...
for i=[loops]
    ...
    hrom1  = plot(real(uR1(1:gap:end)./cR1(1:gap:end)),'b-o','linewidth',5,'markersize',10);
    hrom2  = plot(real(uR2(1:gap:end)./cR2(1:gap:end)),'g-x','linewidth',5,'markersize',9);
    hrom3  = plot(real(uR3(1:gap:end)./cR3(1:gap:end)),'c-v','linewidth',5,'markersize',10);
    hgnat0 = plot(real(uGN0(1:gap:end)./cGN0(1:gap:end)),'-o','linewidth',4,'markersize',4,'color',[0.7,0.7,0.9]);
    hgnat1 = plot(real(uGN1(1:gap:end)./cGN1(1:gap:end)),'-o','linewidth',4,'markersize',8,'color',[0.5,0.2,0.1]);
    hgnat2 = plot(real(uGN2(1:gap:end)./cGN2(1:gap:end)),'--o','linewidth',3.5,'markersize',6,'color',[0.1,0.8,0.2]);
    hgnat3 = plot(real(uGN3(1:gap:end)./cGN3(1:gap:end)),'m-o','linewidth',3,'markersize',4);
    hgnat4 = plot(real(uGN4(1:gap:end)./cGN4(1:gap:end)),'--o','linewidth',2.5,'markersize',3,'color',[0.1,0.1,0.1]);
    hgnat5 = plot(real(uGN5(1:gap:end)./cGN5(1:gap:end)),'-o','linewidth',2,'markersize',2,'color',[0.1,0.55,0.1]);
    hgnat6 = plot(real(uGN6(1:gap:end)./cGN6(1:gap:end)),'-o','linewidth',1,'markersize',1,'color',[0.1,0.55,1]);
    h_legend = legend([hfom,hrom1,hrom2,hrom3,hgnat0,hgnat1,hgnat2,hgnat3,hgnat4,hgnat5,hgnat6],METHODS);
    set(h_legend,'FontSize',12)
    xlabel('spatial domain','FontSize',15)
    ylabel('Mach','FontSize',15)
    ylim([0 5]);
end
...
hgnat6 = semilogy(gnat6.Rnorm,'LineStyle','-','Marker','o','color',[0.1,0.55,1],'linewidth',1,'markersize',1);
list = {'GNATcnstd(PGcnstd)', 'GNATcnstd(GNATcnstd)','GNATcnstd(FOM)','GNATcnstd(GNAT)','GNATcnstd(PG)'};
% list = {'GNATcnstd(PGcnstd)','GNATcnstd(FOM)'};
h_legend = legend([hgnat2,hgnat3,hgnat4,hgnat5,hgnat6],list);
set(h_legend,'FontSize',12,'interpreter','latex')
xlabel('time step','FontSize',15)
ylabel('norm of REAL constr','FontSize',15)
